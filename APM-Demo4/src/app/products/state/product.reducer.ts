...
export function reducer(state = initialState, action: ProductActions): ProductState {
  ...
  switch (action.type) {
    ...
    case ProductActionTypes.UpdateProductSuccess: {
      ...
      return { ...state, products: updatedProducts, currentProductId: action.payload.id };
    }

    // After a create, the currentProduct is the new product.
    case ProductActionTypes.CreateProductSuccess: {
      return { ...state, products: [...state.products, action.payload], currentProductId: action.payload.id };
    }

    // After a delete, the currentProduct is null.
    case ProductActionTypes.DeleteProductSuccess: {
      return { ...state, products: state.products.filter(product => product.id !== action.payload.id), currentProductId: null };
    }
